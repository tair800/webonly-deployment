-- Test the new column collation with Unicode characters
-- Now that we have a Unicode-aware collation, let's test it

-- Test inserting a Unicode record with a valid EquipmentId
INSERT INTO EquipmentFeatures (EquipmentId, Feature, OrderIndex, CreatedAt) VALUES 
(1, N'Test: ə ı ç ş ğ ü ö ı', 99, GETDATE());
...
WHERE Feature LIKE '%ə%';

-- Clean up test data
DELETE FROM EquipmentFeatures WHERE Feature LIKE '%Test:%';

-- Now let's recreate all features with proper Unicode
...
-- Show final results
SELECT 'Features recreated with new collation. Total features: ' + CAST(COUNT(*) AS VARCHAR) AS Result
FROM EquipmentFeatures;

-- Show a sample to verify Unicode is working
SELECT TOP 5
    Id,
    EquipmentId,
    Feature,
